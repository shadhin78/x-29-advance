...
const VisualTrendsSection = dynamic(
  () => import('./VisualTrendsSection').then((m) => m.VisualTrendsSection),
  {
    loading: () => (
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl md:rounded-[2rem] border border-slate-200/50 dark:border-slate-700/50 h-[380px] flex items-center justify-center animate-pulse">
        <span className="text-xs font-bold text-slate-400">Loading Visual Analysis Trends...</span>
      </div>
    ),
  }
);

const SubjectTrendModal = dynamic(
  () => import('./SubjectTrendModal').then((m) => m.SubjectTrendModal),
  { ssr: false }
);

const YearlyActionsModal = dynamic(
  () => import('./YearlyActionsModal').then((m) => m.YearlyActionsModal),
  { ssr: false }
);
  ...
  return (
    ...
    >
      ...
      <VisualTrendsSection
        ...
        dailyActionsData={dailyActionsData}
        habits={habits}
        activePaceGoal={activePaceGoal}
        activePaceStats={activePaceStats}
        activePaceChart={activePaceChart}
        globalPaceStats={globalPaceStats}
        onOpenSubjectTrend={() => setIsSubjectTrendModalOpen(true)}
        onOpenYearlyActions={() => setIsYearlyActionsModalOpen(true)}
      />

      {/* 8. Subject Trend Drilldown Modal */}
      {isSubjectTrendModalOpen && (
        <SubjectTrendModal
          isOpen={isSubjectTrendModalOpen}
          onClose={() => setIsSubjectTrendModalOpen(false)}
      ...
      {isYearlyActionsModalOpen && (
        <YearlyActionsModal
          isOpen={isYearlyActionsModalOpen}
          onClose={() => setIsYearlyActionsModalOpen(false)}
          habits={habits}
        />
      )}
    </div>
